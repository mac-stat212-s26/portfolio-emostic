...
phone_digits <- function(x) {
  digits <- str_remove_all(x, "[^0-9]")  # strip everything except digits
  str_c("(", str_sub(digits, 1, 3), ") ", str_sub(digits, 4, 6), "-", str_sub(digits, 7, 10))
}
...
The error is due to positional matching gone wrong. The first argument of `geom_sf()` is `mapping` and the second is `data`, so R thinks `census_data` is supplying aesthetics. The fix is to name the arguments explicitly: `geom_sf(data = census_data, mapping = aes(fill = population))`.
...
```{r eval=FALSE}
convert_temp <- function(value, unit) {
  if (unit == "C") {
    result <- (value * 9/5) + 32
    cat(value, "C =", result, "F\n")
  } else if (unit == "F") {
    result <- (value - 32) * 5/9
    cat(value, "F =", result, "C\n")
  } else {
...
convert_temp(100, "C")   # 212 F
convert_temp(32, "F")    # 0 C
convert_temp(-40, "C")   # -40 F (fun fact: they meet here!)
```
...
domain_name <- function(email) {
  # check valid email: has @ and ends in .something
  is_valid <- str_detect(email, "^[^@]+@[^@]+\\.[^@]+$")
  if_else(
    is_valid,
    str_extract(email, "(?<=@).+"),  # everything after the @
    "Invalid Email"
  )
}

domain_name("elma@gmail.com")        # "gmail.com"
domain_name("test@macalester.edu")   # "macalester.edu"
domain_name("notanemail")            # "Invalid Email"
...
```{r}
scatter_smooth <- function(df, x_var, y_var) {
...
scatter_smooth(diamonds, x, price)
```
## Done!

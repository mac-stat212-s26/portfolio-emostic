...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  ...
  a. MNAR probably: The chance of missingness is likely due to factors that aren't recorded (like discomfort from the treatment).
  b. Could be MCAR if the missing values are from technical glitches. Could be MAR if the missing values are related to other measured weather variables. Could be MNAR if the values tend to be missing when the values themselves are high (e.g., high temperature causing device malfunction).
  c. Could be MCAR if the bugs affected the platform uniformly. Could be MAR if the bugs affected groups of users differently but where the groupings are known (e.g., mobile vs. desktop). Could be MNAR if the bugs lead to issues affecting users with slower internet where internet speed is not measured.
...
Ozone is missing when Solar.R is at higher values for both alternative situations. In Alt 1, we have some observed data in the range of Solar.R where Ozone is missing, so we could predict it fairly well. In Alt 2, Ozone is missing when Solar.R is beyond the values of our observed data, so we don't have a good sense of the relationship — this would require extrapolation and is concerning.

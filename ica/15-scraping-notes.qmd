...
- Publication date: `.thumbnail-teaser__date` (10 matches)
- Article abstract paragraph (including date): `.thumbnail-teaser__description` (10 matches)
...
nih |>
  ...
  html_attr("class") |>
  unique() |>
  (\(x) x[grepl("teaser|date|abstract|release", x, ignore.case = TRUE)])()
```
```{r}
nih |>
  html_elements(".thumbnail-teaser__body") |>
  head(2) |>
  as.character()
```
...
scrape_nih_page <- function(page_num) {
  url <- paste0("https://www.nih.gov/news-events/news-releases?page=", page_num)
  page <- read_html(url)
  ...
  # Extract date (everything before the dash)
  date <- str_extract(description, "^[^–]+") |> str_trim()
  
  # Extract abstract (everything after the dash)
  abstract <- str_remove(description, "^[^–]+–\\s*") |> str_trim()
  
  data.frame(title = title, date = date, abstract = abstract)
}

# Scrape first 5 pages using map
all_pages <- map(0:4, scrape_nih_page)
nih_df <- list_rbind(all_pages)

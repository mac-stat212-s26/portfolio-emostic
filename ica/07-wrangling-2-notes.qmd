...
```{r ex_create_string}
x <- "It's Thursday" # Need double quotes because of the apostrophe
# x <- 'It's Thursday' # This breaks because the apostrophe ends the string early
```
...
x <- "C:\\Users"
str_view(x)
# x <- "C:\Users" # This breaks because \U is interpreted as a special escape
```
...
# ab? matches "a" then "b" at most once — so "a" and "ab" match, but not "abb"
str_view(c("a", "ab", "abb"), "ab?")
# ab+ requires at least one "b" after "a" — so "a" alone doesn't match
str_view(c("a", "ab", "abb"), "ab+")
# ab* matches "a" followed by any number of b's including zero — all match
str_view(c("a", "ab", "abb"), "ab*")
```
...
```{r}
str_count(posts$question_only, "happi")
str_detect(posts$question_only, "happi")
str_subset(posts$question_only, "happi")
str_extract(posts$question_only, "happi")
str_replace(posts$question_only, "happi", "HAPPI")
str_remove(posts$question_only, "happi")
```
